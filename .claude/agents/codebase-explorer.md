...
description: |
  ...
  <commentary>
  Open-ended codebase question triggers structured exploration: SCAN → MAP → ANALYZE → SYNTHESIZE → REPORT.
  </commentary>
  </example>
  ...
  <commentary>
  Onboarding request triggers architecture deep dive with tech stack mapping and data flow tracing.
  </commentary>
  </example>
...
Run before completing any substantive task. Canonical layer (per `_template.md.example`) PLUS domain-tailored layer below:

### Canonical (template-aligned)
...
[ ] Error cases handled
[ ] Domain-specific check (see below)
